...
document.getElementById("btnExportar").addEventListener("click", () => {
  ...
  document.querySelectorAll(".tabla").forEach(tabla => {
    ...
    //encabezados
    const data = [[
      "N° Vale", "Fecha", "Usuario", "Aprueba",
      "Hora Inicio", "T. Espera", "T. Total", "Hora Final",
      "Origen", "Destino", "Tipo", "Peaje", "Ruta", "Total",
      "Kilometraje(km)","Kilometraje acumulado (km)"
    ]];

    filas.forEach(tr => {
      const celdas = Array.from(tr.querySelectorAll("td")).map(td => td.textContent.trim());
      
      // La celda de "Kilometraje (km)" es la número 14 (índice 13)
      const kmViaje = celdas[14].trim(); // ⬅️ Obtener el valor de la columna 'Km' (índice 14)'
      const totalSoles = celdas[13].replace('S/', '').trim(); // Columna Total (índice 13)
      const kmSinUnidad = kmViaje.replace('S/', '').trim();

      // Dejamos las 13 columnas originales (N° Vale hasta Ruta)
      const celdasFinal = celdas.slice(0, 13); 

      // 2. Reconstruir el final de la fila en el orden del Excel (Total, Km, Acumulado)
      celdasFinal.push(totalSoles); // Columna 14: Total
      celdasFinal.push(kmSinUnidad); // Columna 15: Kilometraje (km)
      
      // 3. Añadir el KM acumulado (que está en el data-km del <tr>)
      const kmAcumulado = tr.getAttribute("data-km") || "0";
      celdas.push(kmAcumulado);

      data.push(celdas);
    });

    const ws = XLSX.utils.aoa_to_sheet(data);
    //const ws = XLSX.utils.table_to_sheet(tabla);  // ⬅️ usar la data construida (con km)
